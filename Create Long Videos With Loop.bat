...
if "%AUDCHOICE%"=="1" (
  ...
  if "!HAS_VIDEO_AUDIO!"=="1" (
    ...
    "%FFM%" -y -hide_banner -loglevel error -stats -i "%VIDEO%" -i "%AUDIO%" ^
      ...
      -map "[v]" -map "[a]" -c:v libx264 -preset ultrafast -crf 25 -c:a aac -b:a 128k -shortest "%MIX%"
  ) else (
    echo (video has NO audio → use only music)
    "%FFM%" -y -hide_banner -loglevel error -stats -i "%VIDEO%" -i "%AUDIO%" ^
      -filter_complex "[0:v]%SCALE_FILTER%format=yuv420p[v]" ^
      -map "[v]" -map 1:a -c:v libx264 -preset ultrafast -crf 25 -c:a aac -b:a 128k -shortest "%MIX%"
  )
)
